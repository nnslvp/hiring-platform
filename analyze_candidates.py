    ...
    "properties": {
        ...
        "profile": {
            ...
            "properties": {
                ...
                "phone_number": {
                    "type": ["string", "null"],
                    "description": "Номер телефона кандидата (если указан)"
                }
            },
            "required": [
                "work_permit_status",
...
⚠️ ОБЯЗАТЕЛЬНОЕ ПРАВИЛО — "документы все есть":

ЕСЛИ кандидат:
1. Говорит "документы все есть" или "все документы есть" или "всё есть"
2. И при этом УЖЕ работает водителем в Польше/ЕС (например: "работаю в Польше", "сейчас работаю", "на данный момент работаю")

ТОГДА ОБЯЗАТЕЛЬНО ставь:
- code_95_status = "есть" (без код 95 нельзя легально работать водителем в ЕС!)
- driver_card_status = "есть" (без карты водителя нельзя легально работать!)
- adr_status = null (ADR — опционально, не у всех)

ПРИМЕР:
Кандидат: "я на данный момент работаю и нахожусь в Польше"
Кандидат: "документы все есть"
→ code_95_status = "есть", driver_card_status = "есть", adr_status = null

Это ЛОГИЧЕСКОЕ СЛЕДСТВИЕ: без код 95 и карты водителя невозможно легально работать водителем грузовика в ЕС.
...
2. code_95_status — код 95 (свидетельство квалификации)
   • "есть": "код 95 есть", ИЛИ "документы все есть" + работает водителем в ЕС
   • "в процессе": "делаю код 95", "код 95 в конце месяца"
   • "нет": ТОЛЬКО если ЯВНО сказал "кода 95 нет"
   • null: не упоминается, неоднозначно, или не работает водителем

3. adr_status — ADR (опасные грузы)
   • "есть": ТОЛЬКО если ЯВНО сказал "ADR есть"
   • "в процессе": "ADR делаю", "ADR будет"
   • "нет": ТОЛЬКО если ЯВНО сказал "ADR нет" или отказался от вакансии из-за ADR
   • null: не упоминается (ADR есть не у всех, "документы все есть" НЕ означает ADR!)

4. driver_card_status — карта водителя (чип для тахографа)
   • "есть": "чип есть", "карта водителя есть", ИЛИ "документы все есть" + работает водителем в ЕС
   • "в процессе": "чип делаю"
   • "нет": ТОЛЬКО если ЯВНО сказал "нет карты"
   • null: не упоминается, неоднозначно, или не работает водителем
